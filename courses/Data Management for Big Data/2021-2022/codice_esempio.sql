...
-- Such an  instruction should be executed each time you reconnect to Postgres
...
What if we wanted now to change something regarding the definition?
...
those that instead operate on the instance of a relation (e.g., ALTER vs UPDATE)
*/
...
column must meet a specific requirement. Such a requirement can be specified by means
of boolean expressions, i.e., expressions that return a truthness value, either "True",
or "False"
...
Thus, we may actually have duplicate rows in our result.
...
-- The number of professors  with a salary greater than 60000 
...
-- Note that, in this case, we are required to use our "table.attribute" notation, in order
...
What is we wanted to extract information regarding all courses and, for those with 
...
-- we extracted all courses of 8550 (requirements) and from them we removed the "situation"
...
that are visible only to the very same user and that get automatically dropped when 
the user disconnects from the database. For example:
*/
...
an UPDATE operation or a DELETE operation.
...
the given operation is actually applied on the table.
...
/*
What about our two constraints? Which kinds of operations could violate them?
	- total participation of "department" to "belongs to"
...
-- Let us take a look at our departments situation
