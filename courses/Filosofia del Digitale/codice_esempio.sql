/* 
To execute the following code, create a new database within a Postgres
Server, connect to it, and then simply run the code in the given order. 
For instance, you can use PgAdmin as a client interface to do that.
*/
...
The value generated by the WHERE clause is a boolean one: True (= the condition is satisfied) or False
...
-- EXERCISE: extract all professors belonging to the department "DMIF" with a salary
-- greater than 50000
